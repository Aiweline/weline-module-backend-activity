/**
 * Weline_BackendActivity：诚实路由 smoke + 后台真实交互 flow（Wave2/3 假用例整治）
 *
 * 说明：openPrimary 会在候选后台路由中探测出「真正渲染出后台内容区(main#main-content)」的入口，
 * 猜测/空白/404 路由不会渲染内容区，从而让 smoke 诚实失败（防假绿），而非靠“无 Fatal”蒙混。
 *
 * @weline-e2e-spec { module: Weline_BackendActivity, type: flow, layer: backend }
 */
const {
  test,
  expect,
...
const CANDIDATE_ROUTES = ["backendactivity","index","config","dashboard"];
...
moduleDescribe(test, MODULE, 'Weline_BackendActivity 后台流程', () => {
  moduleCase(
    test,
  ...
  moduleCase(
    test,
    { module: MODULE, id: 'BACKENDACTIVITY-FLOW-001' },
    '主入口：搜索/筛选或安全控件真实交互',
    async ({ page }) => {
      await loginAsAdmin(page);
      const { route, fatal } = await openPrimary(page);
